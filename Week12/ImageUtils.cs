...
namespace CGPG
{
    public static class ImageUtils
    {
        ...
        public static byte[] ApplyTriangleMask(
            byte[] imageData, 
            int width, 
            int height, 
            float x1, float y1, 
            ...
            float x3, float y3)
        {
            ...
            int px1 = (int)(x1 * width);
            int py1 = (int)(y1 * height);
            int px2 = (int)(x2 * width);
            int py2 = (int)(y2 * height);
            int px3 = (int)(x3 * width);
            int py3 = (int)(y3 * height);

            // Function to check if a point is inside the triangle
            bool IsPointInTriangle(int px, int py, int ax, int ay, int bx, int by, int cx, int cy)
            {
            ...
            for (int y = 0; y < height; y++)
            {
                for (int x = 0; x < width; x++)
                {
                    ...
                    if (IsPointInTriangle(x, y, px1, py1, px2, py2, px3, py3))
                    {
                        // Calculate the index in the flat byte array
                        ...
                        imageData[index + 3] = 255; // A (optional, keep alpha unchanged)
                    }
                }
            }
